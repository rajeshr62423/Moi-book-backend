...
export class DashboardService {
  ...
  async getSummary(userId: string): Promise<DashboardSummaryDto> {
    ...
    const [
      ...
      this.guestModel.countDocuments({ userId: uid, status: 'pending' }),
      this.vendorModel.countDocuments({ userId: uid }),
      this.vendorModel.countDocuments({ userId: uid, status: { $ne: 'booked' } }),
      ...
      this.eventModel.find({ userId: uid }).sort({ createdAt: -1 }).limit(5).exec(),
      this.guestModel.find({ userId: uid }).sort({ createdAt: -1 }).limit(5).exec(),
      this.vendorModel.find({ userId: uid }).sort({ createdAt: -1 }).limit(5).exec(),
      this.moiModel.find({ userId: uid }).sort({ createdAt: -1 }).limit(5).exec(),
      this.ledgerModel.find({ userId: uid }).sort({ createdAt: -1 }).limit(5).exec(),
    ]);

    const totalBudget = budgetAgg[0]?.total ?? 0;
    const totalPaid = paidAgg[0]?.total ?? 0;
    const totalPending = pendingLedgerEntries.reduce((sum, e) => sum + e.amount, 0);
    ...
    const activity: ActivityEntryDto[] = [
      ...recentEvents.map((e) => this.toActivity('events', `Event created: ${e.name}`, e)),
      ...recentGuests.map((g) => this.toActivity('guests', `Guest added: ${g.name}`, g)),
      ...recentVendors.map((v) => this.toActivity('vendors', `Vendor added: ${v.name}`, v)),
      ...recentMoi.map((m) => {
        const guestName = guestNameById.get(m.guestId.toString()) ?? 'a guest';
        const kind = m.type === 'gift' ? m.giftName || 'a gift' : currency(m.amount ?? 0);
        return this.toActivity('moi', `${kind} contribution from ${guestName}`, m);
      }),
      ...recentLedger.map((l) => this.toActivity('ledger', `Transaction logged: ${l.title}`, l)),
    ]
  ...
  private toActivity(icon: ActivityIcon, text: string, doc: unknown): ActivityEntryDto {
